...
SPEED_FLY_UP_MAX = 12 #Tốc độ lớn nhất của viên đạn bay lên (trước khi nỏ)
SPEED_FLY_UP_MIM = 8 #Tốc độ nhỏ nhất của viên đạn bay lên (trước khi nổ)
...
class Bullet(): #Viên đạn sau khi nổ
    def __init__(self, x, y, speed, angle, color):
        ...
        self.y = y
        self.speed = speed
        self.angle = angle #Góc hợp bỏi viên đạn và phương ngang
        self.size = SIZE
        self.color = color
...
class FireWork(): #Quả pháo hoa
    def __init__(self, x, y):
        ...
        def creatBullets(): #Tạo các list các viên đạn
            bullets = []
            color = Random.color()
...
class Random():
    ...
    def randomBulletFlyUp_speed(): #Tốc độ viên đạn bay lên
        speed = random.uniform(SPEED_FLY_UP_MIM, SPEED_FLY_UP_MAX)
        return speed
    def randomBulletFlyUp_x(): #Vị trí viên đạn bay lên
        x = random.randint(int(WINDOWWIDTH*0.2), int(WINDOWNHEIGHT*0.8))
        return x
...
def main():
    ...
    DISPLAYSURF = pygame.display.set_mode((WINDOWWIDTH, WINDOWNHEIGHT))
    fireWorks = []
    time = TIME_CREAT_FW
    BulletFlyUps = []
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            while i < len(BulletFlyUps):
                if BulletFlyUps[i].speed <= 0: #Viên đạn bay lên đạt độ cao tối da
